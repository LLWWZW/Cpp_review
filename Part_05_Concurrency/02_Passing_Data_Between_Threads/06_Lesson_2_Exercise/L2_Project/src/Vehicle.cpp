...
void Vehicle::drive()
{
    ...
    // std::cout << "number of threads of vehicles: " << threads.size() << std::endl; // lw
    ...
    while (true)
    {
        ...
        if (timeSinceLastUpdate >= cycleDuration)
        {
            ...
            if (completion >= 0.9 && !hasEnteredIntersection)
            {
                ...
                // Intersection::addVehicleToQueue, the object _currDestination and a shared pointer to this using the
                // get_shared_this() function. Then, wait for the data to be available before proceeding to slow down.
                // LW: solving
                // std::future<void> ftr = std::async(&Intersection::addVehicleToQueue, i2, this);
                std::future<void> ftrEntryGranted =
                    std::async([i2](std::shared_ptr<Vehicle> v) { i2->addVehicleToQueue(v); }, get_shared_this());
                ftrEntryGranted.get();
                /*standard answer:
