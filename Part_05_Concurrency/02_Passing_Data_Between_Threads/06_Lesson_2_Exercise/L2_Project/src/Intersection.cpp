...
void Intersection::addVehicleToQueue(std::shared_ptr<Vehicle> vehicle)
{
    ...
    // L2.2 : First, add the new vehicle to the waiting line by creating a promise, a corresponding future and then
    // adding both to _waitingVehicles. Then, wait until the vehicle has been granted entry.
    // LW: solving
    std::promise<void> prms;
    std::future<void> ftrs = prms.get_future();
    // WaitingVehicles::pushBack(std::shared_ptr<Vehicle> vehicle, std::promise<void> &&promise);
    _waitingVehicles.pushBack(vehicle, std::move(prms));
    // lw: 负责把此时的 promise和vehicle 放入_waitingVehicles中排队

    ftrs.wait(); // lw: 让当前线程等待，直到与这个 std::future 相关联的异步操作完成。
